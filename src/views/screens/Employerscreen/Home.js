...
React.useEffect(()=>{
 ...
 await axiosRequest.get('auth/post/view/'+ user.id+"/").then((response)=>{
...
<TouchableOpacity onPress={() => navigation.navigate('Settings')}>
  ...
  {gets.post.map((label)=>(
<View key = {label.id} style={[Universalstyles.jobPost,{}]}>
{label.profile.map((profile)=>(
    <View key = {profile.id}style={Universalstyles.jobContent}>
    {profile.profile ? <Image source={{uri : profile.profile}} style={[Universalstyles.Jobimage,{resizeMode:'cover'}]}/> : <Image source={Logo1} style={Universalstyles.Jobimage}/>}
...
{label.status == "close" &&  <OptionsMenu
  ...
  /> }
    ...
    <Text style={{fontSize: 20, borderBottomWidth: 1, marginBottom: 5, borderColor: '#cbc8ce'}}><Icon3 name='person' style={{fontSize: 23, color: 'black',}}/><Text style={{color: 'black', }}>  {label.looking_for}</Text></Text>
    ...
    <Text style={{opacity:.5}}><Icon2 name='account-group' style={{fontSize: 20, color: 'brown', alignContent: 'center'}}/> <Text style={{color: 'black', }}> {label.applies.length} </Text> People applied</Text>
  
<Text style={{opacity: .5 }}><Icon2 name='clock-outline' style={{fontSize: 20, color: 'black', }}/> {moment(label.created_at).startOf('seconds').fromNow()}</Text>

    <TouchableOpacity onPress={()=>{
     setPostID(label.postID)
        navigation.navigate('Manage', { post:label})
